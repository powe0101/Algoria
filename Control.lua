...
function BackgroundMove(_s,dt) --07 21 근영 key 입력 받았을시
  self=_s --tree,cloud에서 self를 받는다
    ...
    if love.keyboard.isDown('right') then
      if pl and pl:GetX()~=WIDTH-30 and pl:GetX()~=0 and pl:GetX() > 225 + BridegePassValue and stageLevel == 1 then --봄일 때.
      ...
      elseif pl and pl:GetX()~=WIDTH-30 and pl:GetX()~=0 and isCanMoveRight then
          self.frame = (self.frame + 15*dt) % 3
          self.x = self.x - MOVE_POWER
      end
    end
