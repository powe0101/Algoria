...
function BackgroundMove(_s,dt) --07 21 근영 key 입력 받았을시
  self=_s --tree,cloud에서 self를 받는다
    ...
    if love.keyboard.isDown('right') then
      if pl and pl:GetX()~=WIDTH-10 and pl:GetX()~=0 and pl:GetX() > 225 + BridegePassValue and stageLevel == 1 then --봄일 때.
    ...
    if love.keyboard.isDown('left')  then
      if pl and pl:GetX()~=WIDTH-10 and pl:GetX()~=0 and pl:GetX() < 460 - BridegePassValue and stageLevel == 3 then
        -- 도개교가 열리기 전까지는 못움직이게 함. 이후 움직이게 하기 위해서는 TRUE/FALSE로 값을 조절해야 할 듯.
