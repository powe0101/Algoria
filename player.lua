...
function Player:UpdateMoveRight(dt)
	...
	elseif self.x < WIDTH - 10  and stageLevel~=2 and isCanMoveRight   then
		self.x = self.x + PLAYER_MOVE_POWER
...
function Player:UpdateMoveLeft(dt)
	...
	elseif self.x > 0  and stageLevel~=2 and isCanMoveLeft then
		self.x = self.x - PLAYER_MOVE_POWER
...
function Player:collideWithPoint2(x,y,_player)
 		if 
         pl:GetX()-5<x and x<pl:GetX()+22 then
             if pl:GetY()<y then
        	return true         
        	end     
   		else
